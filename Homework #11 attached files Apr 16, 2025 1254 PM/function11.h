...
/*  Column mean function with npts is the number of data  */
...
double col_std_dev(double x[NROWS][NCOLS], int npts, int col)
{
    int k;
    double sumvar=0;
    double col_mean(double x[NROWS][NCOLS], int npts, int col);
    ...
    for(k=0;k<=npts-1;k++)
       sumvar+=((col_mean(x, npts, col)-x[k][col])*(col_mean(x,npts,col)-x[k][col]));

    return sqrt(sumvar/(npts-1));
}
